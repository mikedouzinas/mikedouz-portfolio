...
export interface KBChunk {
  ...
  metadata: {
    type: 'profile' | 'experience' | 'project' | 'playground' | 'value' | 'contact' | 'route' | 'fun';
...
async function loadAndChunkKB(): Promise<KBChunk[]> {
  ...
  try {
    ...
    // Process experience
    const experienceData = experience.default;
    experienceData.experiences.forEach((exp: any) => {
      chunks.push({
        id: `chunk-${chunkId++}`,
    ...
    // Process projects
    const projectsData = projects.default;
    ...
    // Process playground
    const playgroundData = playground.default;
    playgroundData.playground_projects.forEach((project: any) => {
      chunks.push({
        id: `chunk-${chunkId++}`,
    ...
    // Process values
    const valuesData = values.default;
    valuesData.core_values.forEach((value: any) => {
      chunks.push({
        id: `chunk-${chunkId++}`,
    ...
    // Process contact
    const contactData = contact.default;
    const contactText = `Contact Mike via ${contactData.contact_methods.map((method: any) => 
      `${method.type}: ${method.value} (${method.description})`
    ).join(', ')}. ${contactData.response_time}. ${contactData.availability}.`;
    ...
    siteMapData.routes.forEach((route: any) => {
      chunks.push({
        id: `chunk-${chunkId++}`,
    ...
    // Process fun facts
    const funData = fun.default;
    const interestsText = funData.interests.map((interest: any) => 
      `${interest.name}: ${interest.description}`
    ).join('. ');
    const hobbiesText = `Hobbies include: ${funData.hobbies.join(', ')}.`;
