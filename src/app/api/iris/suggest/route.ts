...
import { classifyIntent, isAllowedIntent, rewriteToValidQuery } from '@/lib/iris/intents';
...
async function generateHybridSuggestions(
  ...
  while (validSuggestions.length < 5) {
    const defaultSugs = getDefaultSuggestions(signals, 5);
    const existingTitles = validSuggestions.map(s => s.title.toLowerCase());
    
    const newDefaults = defaultSugs.filter(s => 
      !existingTitles.includes(s.title.toLowerCase())
    );
    
    if (newDefaults.length === 0) break;
    
    validSuggestions.push(newDefaults[0]);
  }
